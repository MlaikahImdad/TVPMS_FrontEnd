...
const DataTablePagination = ({
  ...
  const [paginationItms, setPaginationItms] = useState([]);

  useEffect(() => {
    let temp_pageNumbers = []
    for (let i = 1; i <= Math.ceil(totalItems / itemPerPage); i++) {
      temp_pageNumbers.push(i);
    }
    setPageNumbers(temp_pageNumbers);
  ...
  useEffect(() => {
    ...
    setPaginationItms(paginationItms);
  ...
  }, [customItemPerPage, onChangeRowsPerPage,setRowsPerPage]);
